...
func (pc *PermissionCreate) SetRead(b bool) *PermissionCreate {
	pc.mutation.SetRead(b)
	return pc
}

// SetNillableRead sets the "read" field if the given value is not nil.
func (pc *PermissionCreate) SetNillableRead(b *bool) *PermissionCreate {
	if b != nil {
		pc.SetRead(*b)
	}
	return pc
...
func (pc *PermissionCreate) SetCreate(b bool) *PermissionCreate {
	pc.mutation.SetCreate(b)
	return pc
}

// SetNillableCreate sets the "create" field if the given value is not nil.
func (pc *PermissionCreate) SetNillableCreate(b *bool) *PermissionCreate {
	if b != nil {
		pc.SetCreate(*b)
	}
	return pc
...
func (pc *PermissionCreate) SetDelete(b bool) *PermissionCreate {
	pc.mutation.SetDelete(b)
	return pc
}

// SetNillableDelete sets the "delete" field if the given value is not nil.
func (pc *PermissionCreate) SetNillableDelete(b *bool) *PermissionCreate {
	if b != nil {
		pc.SetDelete(*b)
	}
	return pc
...
func (pc *PermissionCreate) AddModuleIDs(ids ...int) *PermissionCreate {
	pc.mutation.AddModuleIDs(ids...)
	return pc
}

// AddModule adds the "module" edges to the Module entity.
func (pc *PermissionCreate) AddModule(m ...*Module) *PermissionCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return pc.AddModuleIDs(ids...)
}
...
func (pc *PermissionCreate) defaults() {
	if _, ok := pc.mutation.Read(); !ok {
		v := permission.DefaultRead
		pc.mutation.SetRead(v)
	}
...
func (pc *PermissionCreate) check() error {
	...
	if _, ok := pc.mutation.Read(); !ok {
		return &ValidationError{Name: "read", err: errors.New(`ent: missing required field "Permission.read"`)}
	}
	if _, ok := pc.mutation.Create(); !ok {
...
func (pc *PermissionCreate) createSpec() (*Permission, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.Read(); ok {
		_spec.SetField(permission.FieldRead, field.TypeBool, value)
		_node.Read = value
	}
	if value, ok := pc.mutation.Create(); ok {
	...
	if nodes := pc.mutation.ModuleIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
