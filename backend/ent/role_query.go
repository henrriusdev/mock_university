...
func (rq *RoleQuery) QueryUsers() *UsersQuery {
...
func (rq *RoleQuery) WithUsers(opts ...func(*UsersQuery)) *RoleQuery {
	query := (&UsersClient{config: rq.config}).Query()
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withUsers; query != nil {
		if err := rq.loadUsers(ctx, query, nodes,
			func(n *Role) { n.Edges.Users = []*Users{} },
			func(n *Role, e *Users) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadUsers(ctx context.Context, query *UsersQuery, nodes []*Role, init func(*Role), assign func(*Role, *Users)) error {
	...
	for _, n := range neighbors {
		fk := n.users_role
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_role" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_role" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
