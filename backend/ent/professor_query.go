...
func (pq *ProfessorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Professor, error) {
	...
	if query := pq.withCareers; query != nil {
		if err := pq.loadCareers(ctx, query, nodes,
			func(n *Professor) { n.Edges.Careers = []*Careers{} },
			func(n *Professor, e *Careers) { n.Edges.Careers = append(n.Edges.Careers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProfessorQuery) loadSubjects(ctx context.Context, query *SubjectQuery, nodes []*Professor, init func(*Professor), assign func(*Professor, *Subject)) error {
	...
	query.Where(predicate.Subject(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(professor.SubjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subject_professor
		if fk == nil {
			return fmt.Errorf(`foreign-key "subject_professor" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subject_professor" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProfessorQuery) loadCareers(ctx context.Context, query *CareersQuery, nodes []*Professor, init func(*Professor), assign func(*Professor, *Careers)) error {
	...
	query.Where(predicate.Careers(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(professor.CareersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
