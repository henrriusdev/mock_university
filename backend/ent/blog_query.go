...
func (bq *BlogQuery) QueryOwner() *UsersQuery {
...
func (bq *BlogQuery) WithOwner(opts ...func(*UsersQuery)) *BlogQuery {
	query := (&UsersClient{config: bq.config}).Query()
...
func (bq *BlogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Blog, error) {
	...
	if query := bq.withOwner; query != nil {
		if err := bq.loadOwner(ctx, query, nodes, nil,
			func(n *Blog, e *Users) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BlogQuery) loadOwner(ctx context.Context, query *UsersQuery, nodes []*Blog, init func(*Blog), assign func(*Blog, *Users)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Blog)
	for i := range nodes {
		if nodes[i].blog_owner == nil {
			continue
		}
		fk := *nodes[i].blog_owner
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "blog_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
