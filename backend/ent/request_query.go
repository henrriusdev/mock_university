...
func (rq *RequestQuery) QueryRequester() *UsersQuery {
...
func (rq *RequestQuery) QueryReceiver() *UsersQuery {
	query := (&UsersClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (rq *RequestQuery) Clone() *RequestQuery {
	...
	return &RequestQuery{
		...
		withRequester: rq.withRequester.Clone(),
		withReceiver:  rq.withReceiver.Clone(),
...
func (rq *RequestQuery) WithRequester(opts ...func(*UsersQuery)) *RequestQuery {
	query := (&UsersClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRequester = query
	return rq
}
...
func (rq *RequestQuery) WithReceiver(opts ...func(*UsersQuery)) *RequestQuery {
	query := (&UsersClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withReceiver = query
	return rq
}
...
func (rq *RequestQuery) loadRequester(ctx context.Context, query *UsersQuery, nodes []*Request, init func(*Request), assign func(*Request, *Users)) error {
	...
	for i := range nodes {
		if nodes[i].users_requests_made == nil {
			continue
		}
		fk := *nodes[i].users_requests_made
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "users_requests_made" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RequestQuery) loadReceiver(ctx context.Context, query *UsersQuery, nodes []*Request, init func(*Request), assign func(*Request, *Users)) error {
	...
	nodeids := make(map[int][]*Request)
	for i := range nodes {
		if nodes[i].users_requests_received == nil {
			continue
		}
		fk := *nodes[i].users_requests_received
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "users_requests_received" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
