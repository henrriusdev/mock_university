...
func (cq *ConfigurationQuery) QueryCycle() *CycleQuery {
	query := (&CycleClient{config: cq.config}).Query()
...
func (cq *ConfigurationQuery) WithCycle(opts ...func(*CycleQuery)) *ConfigurationQuery {
	query := (&CycleClient{config: cq.config}).Query()
...
//		GroupBy(configuration.FieldStartRegistrationSubjects).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Configuration.Query().
//		Select(configuration.FieldStartRegistrationSubjects).
//		Scan(ctx, &v)
...
func (cq *ConfigurationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Configuration, error) {
	...
	if query := cq.withCycle; query != nil {
		if err := cq.loadCycle(ctx, query, nodes, nil,
			func(n *Configuration, e *Cycle) { n.Edges.Cycle = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConfigurationQuery) loadCycle(ctx context.Context, query *CycleQuery, nodes []*Configuration, init func(*Configuration), assign func(*Configuration, *Cycle)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Configuration)
	for i := range nodes {
		if nodes[i].configuration_cycle == nil {
			continue
		}
		fk := *nodes[i].configuration_cycle
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "configuration_cycle" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
