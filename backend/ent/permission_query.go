...
func (pq *PermissionQuery) QueryModule() *ModuleQuery {
	query := (&ModuleClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PermissionQuery) WithModule(opts ...func(*ModuleQuery)) *PermissionQuery {
	query := (&ModuleClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withModule; query != nil {
		if err := pq.loadModule(ctx, query, nodes,
			func(n *Permission) { n.Edges.Module = []*Module{} },
			func(n *Permission, e *Module) { n.Edges.Module = append(n.Edges.Module, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PermissionQuery) loadModule(ctx context.Context, query *ModuleQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Module)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Permission)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Module(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.ModuleColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.permission_module
		if fk == nil {
			return fmt.Errorf(`foreign-key "permission_module" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "permission_module" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
