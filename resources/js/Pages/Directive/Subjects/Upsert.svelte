<script>
  ...
  /** @type {string} */
  let prereq =
    subject?.prerequisites?.map((p) => p.id.toString()).join(",") ?? "";
  ...
  let totalHours =
    Number(practiceHours) + Number(theoryHours) + Number(labHours);
  let semester =
    subject?.semester?.toString() === "0" ? "1" : subject?.semester?.toString();
  ...
  function validateSchedule() {
    ...
    for (let entry of scheduleEntries) {
      ...
      if (accumulatedMinutes > totalAllowedMinutes) {
        alert(
          "La cantidad de horas programadas excede el total de horas permitidas."
        );
        return;
      }
    }
    ...
    if (
      remainingMinutes > 0 &&
      scheduleEntries.every((entry) => entry.from && entry.to)
    ) {
      addEmptyScheduleEntry();
    }
  ...
  <section class="bg-muted/40 flex w-full flex-col p-4">
    <main class="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
      <div class="mx-auto grid max-w-[59rem] flex-1 auto-rows-max gap-4">
        ...
        <form
          method="post"
          action="/directive/subjects/view/submit"
          enctype="multipart/form-data"
          class="grid gap-4 md:grid-cols-[1fr_250px] lg:grid-cols-3 lg:gap-8"
          on:submit="{prepareDataBeforeSubmit}"
        >
          <div
          ...
          >
                ...
                <div class="grid gap-4 lg:grid-cols-5">
                  ...
                  >
                    ...
                    <MaskInput
                      id="creditUnits"
                      unmask="none"
                      imask="{{ mask: Number, min: 0, max: 10 }}"
            ...
            <Card.Root>
              <Card.Header>
                ...
                <Card.Description>
                  Fill the form with the subject's class schedule, respecting
                  ...
                  {#each scheduleEntries as { daySelected, from, to }, index}
                    ...
                    >
                      ...
                      <Select.Root
                        ...
                        on:change="{(e) =>
                          (scheduleEntries[index].day = e.detail)}"
                      >
                        <Select.Trigger>
                    ...
                    >
                      ...
                      <MaskInput
                        id="classScheduleFrom{index}"
                        imask="{{
                          mask: 'HH:MM',
                          blocks: {
                            HH: { mask: IMask.MaskedRange, from: 7, to: 18 },
          ...
          <div class="grid auto-rows-max items-start gap-4 lg:gap-8">
            <Card.Root>
              <Card.Header>
                ...
                <Card.Description>
                  Fill the form with the professor, prerequisites and careers
                  (can be one or more) of the subject.
                </Card.Description>
              </Card.Header>
              <Card.Content>
